...
public final class FactoryHolder {
    ...
    private static final Map<ClassLoader, LoggingAdmin> instances = new WeakHashMap<>();
    private static final Map<Object, Object> tokensByLoggerContext = new WeakHashMap<>();

    static {
        ServiceLoader.load(LoggingAdminFactory.class, FactoryHolder.class.getClassLoader())
    ...
    public static LoggingAdmin getLoggingConfigurationAdmin(Object token, ClassLoader classLoader) {
        lock.lock();
        try {
            LoggingAdmin instance = instances.get(classLoader);
            if (instance == null) {
                instance = factories.stream()
                        ...
                        .orElseThrow(() ->
                                new IllegalStateException("No " + LoggingAdminFactory.class.getName() + " found."));
